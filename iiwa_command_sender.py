...
class IiwaCommandSender(LeafSystem):
    def __init__(self ):
        ...
        #Declare input ports
        self.input_port1 = self.DeclareInputPort("position", PortDataType.kVectorValued, self.num_joints)
        self.input_port2 = self.DeclareInputPort("torque", PortDataType.kVectorValued, self.num_joints)
        #Declare output port
        #self.output_port = self.DeclareAbstractOutputPort("lcmt_iiwa_command",AbstractValue.Make(lcmt_iiwa_command),self.CalcOutput)
        self.output_port = self.DeclareAbstractOutputPort("lcmt_iiwa_command",lambda: AbstractValue.Make(lcmt_iiwa_command),self.CalcOutput)


    def CalcOutput(self,context,output):
        
        #isPosition =True #self.input_port1.HasValue(context)
        #if(isPosition):
